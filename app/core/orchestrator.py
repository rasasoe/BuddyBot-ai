...
class Orchestrator:
    ...
    def _fast_local_response(message: str) -> Optional[str]:
        ...
        if "코딩" in text and any(word in text for word in ["뭐", "무엇", "설명", "알려", "뜻"]):
            return "코딩은 컴퓨터에게 일을 시키기 위해 순서와 규칙을 글로 적는 작업입니다. 쉽게 말하면 프로그램을 만드는 방법이에요."

        if any(word in text for word in ["뭐하면좋", "뭐하지", "심심", "할거추천", "할일추천"]):
            return "가볍게 산책하거나 물 한 잔 마시고, 오늘 할 일을 하나만 정해보는 게 좋아요. 원하면 제가 짧게 계획도 같이 잡아드릴게요."

        if any(word in text for word in ["뭐먹", "메뉴추천", "밥추천", "저녁추천", "점심추천"]):
            return "부담 없게 먹으려면 김치볶음밥이나 덮밥이 좋고, 든든하게 먹고 싶으면 국밥이나 파스타도 좋아요."

        if any(word in text for word in ["너랑대화", "대화해도", "얘기하자", "심심해"]):
            return "물론이죠. 짧게 편하게 이야기할 수 있어요. 오늘 있었던 일이나 궁금한 걸 말해 주세요."
